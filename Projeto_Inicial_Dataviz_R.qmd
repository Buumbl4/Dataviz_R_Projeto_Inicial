...
```{r}
vendas <- vendas %>% mutate(total_da_compra = vendas$preco_unitario * vendas$quantidade + vendas$frete)
vendas
```
...
```{r}
ggplot(vendas, aes(x = categoria, y = total_da_compra))+
  geom_col ()+
  labs(
    title = 'Faturamento por Categoria',
    x = 'Categoria', y = 'Faturamento Total (R$)'
  )
  theme_minimal()
...
```{r}
ggplot(clientes_unicos, aes(x = idade))+
...
ggplot(aes(x = regiao, fill = regiao))+
  geom_bar()+
  labs(
    title = 'Proporção Região x Participantes Cashback',
    x = 'Região', y = 'Quantidade de Participantes em Porcentagem'
  )
...
ggplot(aes(x = regiao, fill = regiao))+
  ...
  labs(
    title = 'Proporção Região x Participantes Cashback',
    x = 'Região', y = 'Quantidade de Participantes em %'
  )
```
...
```{r}
g <- ggplot(vendas, aes(x = reorder(categoria, -total_da_compra, FUN = sum), y = total_da_compra)) +
  geom_col()
...
```{r}
ggplot(vendas, aes(x = mes, y = total_da_compra))+
  geom_line()
```
...
```{r}
vendas_por_mes <- vendas %>% group_by(mes) %>% summarise(vendas_por_mes = sum(total_da_compra))
vendas_por_mes
```
...
```{r}
ggplot(vendas_por_mes, aes(x = mes, y = vendas_por_mes, group = 1))+
  geom_line(linewidth = 2, color = 'green', alpha = 0.5)+
  ylim(0, 2e6)
```
...
```{r}
ggplot(vendas_por_mes, aes(x = mes, y = vendas_por_mes, group = 1))+
  geom_line(linewidth = 2, color = 'green', alpha = 0.3)+
  ylim(0, 2e6)+
  geom_point(size = 4, color = 'red', shape = 'diamond')
```
...
```{r}
vendas <- vendas %>% mutate(trimeste = as.factor(quarter(data)))
vendas
```
...
```{r message=FALSE}
vendas_por_tri <- vendas %>% group_by(trimeste, metodo_pagamento) %>% summarise(total_tri = sum(total_da_compra))
vendas_por_tri
```
...
```{r}
ggplot(vendas_por_tri, aes(x = trimeste, y = total_tri, fill = metodo_pagamento))+
...
grafico_vendas_mes <- 
  ggplot(vendas_por_mes, aes(x = mes, y = vendas_por_mes, group=1)) +
    geom_line(linewidth = 2, color = blue_2, alpha = 0.3) +
...
grafico_vendas_mes <- grafico_vendas_mes + 
  tema +
  labs(
    title = 'Comparação Mensal do Total de Vendas da Zoop (2023)',
    x = 'Mês', y = 'Vendas popr Mês (R$)'
  )
...
grafico_vendas_categoria <- 
  ggplot(vendas, aes(x = reorder(categoria, -total_da_compra, FUN = sum), y = total_da_compra)) + 
    geom_col(fill = red_2) +
...
```{r}
grafico_vendas_pag <- 
  ggplot(vendas_por_tri, aes(trimeste, total_tri, fill = metodo_pagamento)) +
  geom_col(position = "dodge") +
  scale_fill_manual(values = cores) +
  tema
...
```{r}
ggsave('imagens/grafico_vendas_mes.png', grafico_vendas_mes, height = 5, width = 7, dpi = 300)
ggsave('imagens/grafico_vendas_categoria.png', grafico_vendas_categoria, height = 5, width = 7, dpi = 300)
ggsave('imagens/grafico_vendas_pag.png', grafico_vendas_pag, height = 5, width = 7, dpi = 300)
```
